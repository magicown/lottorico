...
function apms_video_id($url, $vid, $type) {
	...
	if($type == "nate") {
		...
		switch($type) {
			...
			case 'soundcloud'	: $name = 'property'; $key = 'twitter:player'; $value = 'content'; break;
			default				: $name = ''; break;
		}
...
function apms_video_info($video_url) {
	...
	// 미디어파일일 경우(확장자 체크 && jwplayer)
	$filename = basename($info['path']);
	if($filename) {
		$fileinfo = apms_get_filename($filename);
		if($fileinfo['ext']) {
			$media_ext = array("mp4", "m4v", "f4v", "mov", "flv", "webm", "acc", "m4a", "f4a", "mp3", "ogg", "oga");
			if(in_array($fileinfo['ext'], $media_ext)) {
				$video['type'] = 'file';
				$video['vid'] = 'file';
				return $video;
			}
	...
	if($info['host'] == "youtu.be") { //유튜브
		...
		$vquery = explode("/", $vtmp[1]);
		$video['vid'] = $vquery[0];
	} else if($info['host'] == "soundcloud.com") { // soundcloud.com
...
function apms_video($vid, $opt='') {
	...
	if($video['type'] == "file") { //JWPLAYER
		...
		if($video['type'] == "youtube") { //유튜브
			...
			$show = '<iframe src="'.str_replace("http:", "https:", $video['play_url']).'" width="'.$video['width'].'" height="'.$video['height'].'" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"'.$fullscreen.'></iframe>';
			...
			$show = '<embed src="https://player.yinyuetai.com/video/player/'.$video['vid'].'/v_0.swf" quality="high" width="'.$video['width'].'" height="'.$video['height'].'" align="middle" allowScriptAccess="sameDomain" allowfullscreen="true" type="application/x-shockwave-flash"></embed>';
		} else if($video['type'] == "vlive"){ // www.vlive.tv
			$autoplay = ($autoplayvideo || $video['auto']) ? '?autoPlay=true' : '';
			$show = '<iframe src="https://www.vlive.tv/embed/'.$video['vid'].$autoplay.'" width="'.$video['width'].'" height="'.$video['height'].'" frameborder="no" scrolling="no" marginwidth="0" marginheight="0"'.$fullscreen.'></iframe>';
		} else if($video['type'] == "srook"){ // www.srook.net
			list($author, $key) = explode("_", $video['vid']);
			$show = '<iframe src="https://www.srook.net/nemo_embed/srookviewer.html?author='.$author.'&key='.$key.'&btype=0'.$video['pageNo'].'" width="'.$video['width'].'" height="'.$video['height'].'" frameborder="no" scrolling="no" marginwidth="0" marginheight="0"'.$fullscreen.'></iframe>';
		...
		} else if($video['type'] == "soundcloud"){ // soundcloud.com
			$vpath = str_replace("-", "/", $video['vid']);
			list($vtype, $vcode) = explode("/", $vpath);
			if(G5_IS_MOBILE) {
				$show = '<div style="margin:0 auto 15px; max-width:435px;">';
				if($vtype == 'playlists') {
					$show .= '<iframe width="100%" height="300" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/'.$vpath.'&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true&visual=true"></iframe>';
				} else {
					$show .= '<iframe width="100%" height="400" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/'.$vpath.'&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true&visual=true"></iframe>';
				}
				$show .= '</div>';
			} else {
				$show = '<div style="margin:0 auto 15px;">';
				if($vtype == 'playlists') {
					$show .= '<iframe width="100%" height="450" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/'.$vpath.'&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true"></iframe>';
				} else {
					$show .= '<iframe width="100%" height="166" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/'.$vpath.'&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true"></iframe>';
				}
				$show .= '</div>';
			}
		}

		if($is_feedvideo || $video['type'] == "soundcloud") {
			return $show;
		}
...
function apms_photo_url($mb_id='') {
	...
	$photo_url = G5_DATA_URL.'/member_image/'.$mb_dir.'/'.$mb_id.'.gif';
	$photo_file = G5_DATA_PATH.'/member_image/'.$mb_dir.'/'.$mb_id.'.gif';
	...
	for($i=0;$i < $imgs_cnt;$i++) {
		...
		if($is_destfile) {
			if($mode == 'copy') { //다른이름으로 복사
				...
				$content = str_replace($imgs[$i], $save_url, $content);
			} else { //이미지 삭제
    ...
    switch($sns) {
		...
		case 'kakaotalk':
            if($config['cf_kakao_js_apikey']) {
				...
				$str = 'kakaolink_send(\''.$sns_msg.'\',\''.$sns_url.'\',\''.$thumb_url.'\'); return false;';
...
// 소셜로그인 아이콘 출력
function apms_social_login_icon() {
	global $config, $urlencode;

	if(!$config['cf_social_login_use']) return;
	
	$skin_path = get_social_skin_path();
	$skin_url = get_social_skin_url();

	ob_start();
	include($skin_path.'/social_outlogin.skin.1.php');
	$social_icon = ob_get_contents();
	ob_end_clean();
