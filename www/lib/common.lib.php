...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || $mb_id == 'phpprogram' || $mb_id == '2019051300') return 'super';
...
//회원등급
function makeMemberGrade($mb_id){
    $que = "SELECT pay_name, pay_money, pay_memo, a.mb_2 FROM g5_member a LEFT JOIN g5_pay_type b ON a.mb_10 = b.idx WHERE a.mb_id = '{$mb_id}'";
    $row = sql_fetch($que);
    if($row[pay_name]){
        return $row;
    } else {
...
//회원별 번호 생성 갯수
function getMemberMadeNumCnt($mb_id){
    $que = "SELECT mb_10, mb_3 FROM g5_member WHERE mb_id = '{$mb_id}' AND mb_leave_date = '' ";
    $rs = sql_fetch($que);
    $cnt = 20;
    if($rs['mb_10']){
        if($rs[mb_3]>0){
...
//이번주 번호할당 남은 갯수
function getWeekRemindNumCnt($mb_id){
    //회원별 번호 조합 갯수
    $mb_cnt = getMemberMadeNumCnt($mb_id);
    $turn = getLastTurn();
    ...
    if(!$rs[cnt]){
        ...
        return $mb_cnt - $rs[cnt];
    }
}


//휴대폰 메세지 전송 로그
function save_sms_log($mb_id,$turn,$msg,$hp,$sms_result){
    $que  = "INSERT INTO g5_sms_log SET ";
    $que .= "mb_id          = '{$mb_id}', ";
    $que .= "sms_turn       = '{$turn}', ";
    $que .= "sms_content    = '{$msg}', ";
    $que .= "sms_hp         = '{$hp}', ";
    $que .= "sms_result     = '{$sms_result}',";
    $que .= "sms_regdate    = NOW() ";
    //echo $que."<br><br>";
    sql_query($que);
}
...
function makeLottoNumPay($except, $sms_cnt, $today_cnt, $mb_id, $lt){

    //오늘의 요일을 구한다.
    $sms_today_cnt = $today_cnt;//금일 보낼 sms 갯수
    $que = "SELECT COUNT(*) AS cnt FROM member_lotto_num WHERE lotto_turn = '{$lt}' AND mb_id = '{$mb_id}' AND direct_send = 'N' ";
    //echo $que."<br>";
    $row = sql_fetch($que);
    //echo "보낸 문자 갯수->".$row['cnt'];
    //echo "<br>";

    $remind_sms_cnt = $sms_cnt - $row['cnt'];//현재 남은 문자 갯수
    if($remind_sms_cnt >= $sms_today_cnt){//남은 문자 갯수가 보낼 문자 갯수보다 크다면
        $mb_cnt = $sms_today_cnt;
    } else if($remind_sms_cnt < $sms_today_cnt){//남은 문자 갯수가 보낼 문자 갯수보다 작으면 남은 갯수만큼만 번호를 생성하고 문자를 보낸다.
        $mb_cnt = $remind_sms_cnt;
    }
    ...
    $chk = $data = array();
    $confirm = 0;
    $mabangjin_cnt = $except_cnt = $pattern_cnt = 0;
    while($total<$mb_cnt) {
        $rand = mt_rand(1, $total_cnt);
        if (!isset($chk[$rand])) {
            $chk[$rand] = 1;
            $que = "SELECT idx, no1,no2,no3,no4,no5,no6 FROM lotto_num WHERE idx = {$rand} AND use_yn = 'N' AND set_filter_yn = 'Y'";
            //echo $que."<br>";
            $row = sql_fetch($que);
            if($row['idx']!='') {
                $org_num = array($row[no1],$row[no2],$row[no3],$row[no4],$row[no5],$row[no6]);
                $mabangjin_cnt = mabangjin($org_num); //마방진
                $except_cnt = num_except($org_num, $except); //제외수
                $pattern_cnt = filter_pattern($org_num);
                if($mabangjin_cnt == 0 && $except_cnt == 0 && $pattern_cnt == 0){
                    $data[] = $row;
                    $total++;
                }
            }
...
function makeLottoNumPayOrg($except, $sms_cnt, $today_cnt, $mb_id, $lt){
...
function makeLottoNumPayOld($except, $sms_cnt, $today_cnt, $mb_id, $lt){
    ...
    //연번확인 하기
    $sql  = "SELECT * FROM filter_config WHERE idx = 1";
    $filter = sql_fetch($sql);
    ...
    while($total<$mb_cnt){
        ...
        if (!isset($chk[$rand])) {
            ...
            if($filter['chain_is_1']=='Y') {//연번 모두 제외한다.
                $que = "SELECT idx, no1,no2,no3,no4,no5,no6 FROM lotto_num WHERE idx = {$rand} AND use_yn = 'n' AND chain_is_1 = 'N' AND chain_is_2 = 'N' AND chain_is_3 = 'N' AND chain_is_4 = 'N' AND chain_is_5 = 'N' AND chain_pair_yn = 'N' ";
            } else {
                //1연번 포함한다.
                $que = "SELECT idx, no1,no2,no3,no4,no5,no6 FROM lotto_num WHERE idx = {$rand} AND use_yn = 'n' AND chain_is_2 = 'N' AND chain_is_3 = 'N' AND chain_is_4 = 'N' AND chain_is_5 = 'N' ";
            }
            //echo $que."<br>";
            $row = sql_fetch($que);
...
function setFilter(){
    ...
    //연번
    if($filter['chain_is_1']=='N') {
        $search_filter .= " AND chain_is_1 = 'N' ";
    }
    if($filter['chain_is_2']=='N') {
        $search_filter .= " AND chain_is_2 = 'N' ";
    }
    if($filter['chain_is_3']=='N') {
        $search_filter .= " AND chain_is_3 = 'N' ";
    }
    if($filter['chain_is_4']=='N') {
        $search_filter .= " AND chain_is_4 = 'N' ";
    }
    if($filter['chain_is_5']=='N') {
        $search_filter .= " AND chain_is_5 = 'N' ";
    }

    //이전 1등
    if($filter['before_hit_1']=='N'){
        $search_filter .= " AND before_hit_1 = 'N' ";
    }
    if($filter['before_hit_2']=='N'){
        $search_filter .= " AND before_hit_2 = 'N' ";
    }
    if($filter['before_hit_3']=='N'){
        $search_filter .= " AND before_hit_3 = 'N' ";
    }
    ...
    $ct = 0;
    for($i=0;$i<7;$i++){
        $odd_arr[$ct] = $filter['odd'.$i];
        $ct++;
    }
    $odd_list = implode(",",$odd_arr);
    $search_filter .= " AND odd_cnt NOT IN ({$odd_list})";



    //당번홥계
    if($filter['add_sum_yn']=='Y'){
        $search_filter .= " AND num_sum BETWEEN '{$filter['add_sum_start']}' AND '{$filter['add_sum_end']}' ";
    }
    //끝수합계
    if($filter['last_sum_yn']=='Y'){
        $search_filter .= " AND num_last_sum BETWEEN '{$filter['last_sum_start']}' AND '{$filter['last_sum_end']}' ";
    }
    //AC값
    if($filter['ac_yn']=='Y'){
        $search_filter .= " AND ac BETWEEN '{$filter['ac_start']}' AND '{$filter['ac_end']}' ";
    }
    ...
    $ct = 0;
    for($i=0;$i<7;$i++){
        $high_arr[$ct] = $filter['high'.$i];
        $ct++;
    }
    $high_list = implode(",",$high_arr);
    $search_filter .= " AND number_high NOT IN ({$high_list})";
...
//즉시전송시
function makeLottoNumPayOne($except, $sms_cnt, $today_cnt, $mb_id, $turn, $sel_cnt){

    //오늘의 요일을 구한다.
    $sms_today_cnt = $today_cnt;//금일 보낼 sms 갯수
    $que = "SELECT COUNT(*) AS cnt FROM member_lotto_num WHERE lotto_turn = '{$turn}' AND mb_id = '{$mb_id}'";
    $row = sql_fetch($que);

    $remind_sms_cnt = $sms_cnt - $row['cnt'];

    if($remind_sms_cnt >= $sel_cnt){//남은 문자 갯수가 보낼 문자 갯수보다 크다면
        $mb_cnt = $sel_cnt;
    } else if($remind_sms_cnt < $sel_cnt){//남은 문자 갯수가 보낼 문자 갯수보다 작으면 남은 갯수만큼만 번호를 생성하고 문자를 보낸다.
        $mb_cnt = $remind_sms_cnt;
    }
...
//제외수 확인해서 가져온다.
function getExceptNumCron($turn, $except){
...
//sms전송한다.
function SendMesg($rev_hp, $rev_msg) {
    $rev_hp = str_replace("-","",$rev_hp);
    $query = array(
        'id' => 'lotto645_http',
    ...
    if ($xml === false) {
        ...
        return $xml->messages->message->err_code;
    }
}

function getLastTurn(){
    $que = "SELECT MAX(hit_turn) AS turn FROM lotto_hit_result WHERE 1 LIMIT 1";
    $row = sql_fetch($que);
    return $row[turn]+1;
}

function getLastTurnMemberCnt($mb_id){
    $que = "SELECT COUNT(hit_turn) AS turn FROM lotto_hit_result WHERE 1 LIMIT 1";
    $row = sql_fetch($que);
    return $row[turn]+1;
}

function get_current_hit_turn(){
    $que = "SELECT MAX(hit_turn) AS turn FROM lotto_hit_result WHERE 1 LIMIT 1";
    $row = sql_fetch($que);
    return $row[turn];
}

function getPayType($mb_10){
    $que = "SELECT * FROM g5_pay_type WHERE idx = '{$mb_10}'";
    //echo $que."<br>";
    $row = sql_fetch($que);
    return $row;
}

function getPayTypeText($mb_10){
    $que = "SELECT pay_name FROM g5_pay_type WHERE idx = '{$mb_10}'";
    //echo $que;
    $row = sql_fetch($que);
    if($row['pay_name']==''){
        $grade = "무료회원";
...
function makeLottoNumHItCss($num, $hit){
...
function payUpProc(){
    ...
    echo $post_data["timestamp"]     = date("YmdHis");
    echo "<br>";

    echo $sig = hash('sha256',$id.'|'.$post_data["orderNumber"].'|'.(int)$post_data["amount"].'|'.$pkey.'|'.$post_data["timestamp"]);
    //echo strlen($sig);
...
function regCounselor($mb_id, $mb_hp, $mb_name, $type){
    $que  = "INSERT INTO g5_counselor SET ";
    $que .= "c_mb_id    = '{$mb_id}', ";
    $que .= "c_mb_name  = '{$mb_name}', ";
    $que .= "c_mb_hp    = '{$mb_hp}', ";
    $que .= "c_reg_type = '{$type}', ";
    $que .= "c_regdate  = NOW() ";
    //echo $que."<br>";
    ...
    } else if($type == 'L'){//랜딩 페이지 무료번호 받기 신청시 등록됨

    }
}

//무료번호 생성하기
function makeFreeLottoNum($mb_id, $mb_hp){
    //금주에 회차를 구한다.
    ...
    if($mb_hp) {
        $msg = "{$turn}회차 1등 예상번호 로또리코 체험플랜\n{$sm}\n\n무료거부 (080-855-6398)";
        // 한줄로 이어쓰기 하세요.
        $sms_result = SendMesg($mb_hp,$msg);
        save_sms_log($mb_id,'',$msg,$mb_hp,$sms_result);
    }
    sql_query("COMMIT");
...
function getMemberInfo($mb_id){
    $que = "SELECT * FROM g5_member WHERE mb_id = '{$mb_id}'";
    $row = sql_fetch($que);
    return $row;
}

//결제 후 회원등급 변경
function changeMemberGrade($mb_id, $mb_pay, $mb_money){
    $sql = "SELECT * FROM g5_pay_type WHERE idx = {$mb_pay} ";
    $row = sql_fetch($sql);
    $end_date = date("Y-m-d",strtotime("-1 day",strtotime("+{$row['pay_date']} months")));

    $mb = getMemberInfo($mb_id);

    $que  = "UPDATE g5_member SET ";
    $que .= "mb_2 = '{$end_date}', ";
    $que .= "mb_10 = '{$mb_pay}' ";
    $que .= " WHERE mb_id = '{$mb_id}' ";
    $res = sql_query($que);
    if($res){
        $que1  = "INSERT INTO g5_member_grade_log SET ";
        $que1 .= "log_mb_id         = '{$mb_id}', ";
        $que1 .= "log_org_grade     = '{$mb['mb_10']}', ";
        $que1 .= "log_chg_grade     = '{$mb_pay}', ";
        $que1 .= "log_change_id     = '{$_SESSION['ss_mb_id']}', ";
...
function get_current_turn(){
    ...
    //echo $sql."<br>";
    $rs = sql_fetch($sql);
    return $turn = $rs['hit_turn']+1;
}

//회원남은 번호 갯수
function remind_take_num($mb_id, $total_cnt){
    $turn = get_current_turn();
    //전송된 문자 갯수를 구한다.
    $que = "SELECT COUNT(*) AS cnt FROM member_lotto_num WHERE mb_id = '{$mb_id}' AND lotto_turn = '{$turn}'";
    //echo $que;
    $row = sql_fetch($que);
    return $total_cnt - $row['cnt'];
}

//회원등급변경 로그 만들기
function change_member_grade($mb_id, $cur_grade, $change_grade){
    $que = "INSERT INTO g5_member_grade_log SET ";
    $que .= "log_mb_id      = '{$mb_id}', ";
    $que .= "log_org_grade  = '{$cur_grade}', ";
    $que .= "log_chg_grade  = '{$change_grade}', ";
...
//회원등급 변경시 종료 일자 변경하기
function change_member_expire_date($mb_id, $pay_idx){
    $que = "SELECT pay_date FROM g5_pay_type WHERE pay_idx = '{$pay_idx}'";
    $row = sql_fetch($que);
    if($row['pay_date']>0){
...
function get_id_hit_grade($grade, $mb_id){

    $sql = "
    SELECT COUNT( * ) AS cnt
    FROM member_lotto_num
    WHERE mb_id =  '{$mb_id}'   
    AND lotto_result = '{$grade}'
    GROUP BY lotto_result    
   " ;
    //echo $sql."<br>";
...
//현재 회원등급별 결제 이름 구하기
function get_member_pay_name($mb_id){
    $que2 = "SELECT pay_name, b.mb_hp, b.mb_5 FROM g5_pay_type a left join g5_member b ON a.idx = b.mb_10 WHERE b.mb_id = '{$mb_id}'";
    //echo $que2 . "<br>";
    $row2 = sql_fetch($que2);
    return $row2;
}
...
{
    $temp = array("1", "2", "4", "5", "6", "8", "10", "15", "19", "20", "30", "32", "33", "34", "35", "40", "45", "49", "50", "51", "55", "56", "57", "58", "59", "60", "62", "65", "67", "69", "70");
    ...
    for ($i = 0; $i <= 30; $i++) {
$rem = 0;
        echo $rem = abs($temp[$i]-($temp[$i+1])).":".$cnt1."<br>";
        if($rem == 1){
            ++$cnt1;
        } else {
            echo "초기화<br>";
            $cnt1 = $cnt2 = $cnt3 = $cnt4 = $rem= 0;
        }
    }
...
//남은조합갯수
function cur_remind_lotto_cnt($mb_id, $turn, $send_cnt){
    $que = "SELECT COUNT(*) as cnt FROM member_lotto_num WHERE lotto_turn = '{$turn}' AND mb_id = '{$mb_id}'  ";
    //echo $que."<br>";
    $send_lotto_num = sql_fetch($que);


    $remind_lotto_cnt = $send_cnt-$send_lotto_num['cnt'];
    if($remind_lotto_cnt<0){
        $remind_lotto_cnt = 0;
    }

    return $remind_lotto_cnt;

}

//금일 생성된 조합 갯수
function make_lotto_num($mb_id, $turn){
    $que = "SELECT COUNT(*) AS cnt FROM member_lotto_num WHERE mb_id = '{$mb_id}' AND lotto_turn = '{$turn}'";
    $row = sql_fetch($que);
    return $row['cnt'];
}
...
//필터링 적용시 데이터 업데이트 한다.
function filterChangeUpdateData(){
    $search_query = setFilter();

    //새로운 필터를 적용하려면 기존에 적용된 필터를 N 업데이트 한다.
    $que = "UPDATE lotto_num SET set_filter_yn = 'N' WHERE set_filter_yn = 'Y' ";
    echo $que."<br>";
    sql_query($que);
...
//마방진 패턴 확인
function mabangjin($lotto_num){
    $que = "SELECT * FROM filter_config WHERE idx  = 1";
    $row = sql_fetch($que);
    $flg_false = 0;
    for($i=1;$i<6;$i++) {
        unset($org_num);
        if($row['ma'.$i.'_yn'] = 'Y' && !$row['ma' . $i . '_value']) {
            $org_num = array($row['ma' . $i . '_value']);
            $intens = array_intersect($lotto_num,$org_num);
            if($row['ma'.$i.'_end'] < count($intens)){
                $flg_false++;
                break;
            }
        }
...
//제외수 확인 하기
function num_except($lotto_num, $except){
    $false = 0;
    //제외수 확인
    if ($except) {
        if (in_array($lotto_num, $except)) {
            $false++;
        }
    }
    return false;
...
//패턴 확인
function filter_pattern($lotto_num){
    $que = "SELECT * FROM filter_config WHERE idx  = 1";
    $row = sql_fetch($que);
    $flg_false = 0;
    for($i=1;$i<=42;$i++) {
        unset($org_num);
        if($row['pattern'.$i.'_yn'] = 'Y' && !$row['pattern' . $i . '_value']) {
            $org_num = array($row['ma' . $i . '_value']);
            $intens = array_intersect($lotto_num,$org_num);
            if(count($intens)>5){
                $flg_false++;
