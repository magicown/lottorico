...
function apms_item_rows($arr) {
	...
	$sql_pt = '';
	if($pt_list) {
		$sql_pt = ($ex_pt) ? "and find_in_set(pt_id, '{$pt_list}')=0" : "and find_in_set(pt_id, '{$pt_list}')";
	}
	...
	$sql_common = "from {$g5['g5_shop_item_table']} where it_use = '1' and it_soldout <> '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term $sql_main $sql_where";
	if($page > 1) {
...
function apms_item_partner_rows($arr) {
	...
	$sql_common = "from {$g5['g5_shop_item_table']} where it_use = '1' and it_soldout <> '1' and pt_id <> '' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term $sql_main $sql_where";
	if($page > 1) {
...
function apms_sale_rows($arr) {
	...
	$sql_pt = '';
	if($pt_list) {
		$sql_pt = ($ex_pt) ? "and find_in_set(b.pt_id, '{$pt_list}')=0" : "and find_in_set(b.pt_id, '{$pt_list}')";
	}
	...
	$sql_common = "from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id ) 
					where b.it_use = '1' and b.it_soldout <> '1' and a.ct_status = '완료' and a.ct_select = '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term ";
...
function apms_sale_partner_rows($arr) {
	...
	$sql_common = "from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id ) 
					where a.pt_id <> '' and a.ct_status = '완료' and a.ct_select = '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term ";
...
function apms_mkt_partner_rows($arr) {
	...
	$sql_pt = '';
	if($pt_list) {
		$sql_pt = ($ex_pt) ? "and find_in_set(b.mk_id, '{$pt_list}')=0" : "and find_in_set(b.mk_id, '{$pt_list}')";
	}
	...
	$sql_common = "from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id ) 
					where a.mk_id <> '' and a.ct_status = '완료' and a.ct_select = '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term ";
...
function apms_item_event_rows($arr) {
	...
	$sql_common = "from `{$g5['g5_shop_event_item_table']}` a left join `{$g5['g5_shop_item_table']}` b on (a.it_id = b.it_id) 
					where a.ev_id = '{$ev_id}' and b.it_use = '1' and b.it_soldout <> '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term ";
...
function apms_item_post_rows($arr) {
	...
	if($mode == 'comment') {
		...
		$sql_common = "from `{$g5['apms_comment']}` a join `{$g5['g5_shop_item_table']}` b on (a.it_id=b.it_id)
						where b.it_use = '1' and b.it_soldout <> '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term ";		
	} else if($mode == 'use') {
		$orderby .= "a.is_id desc";
		$sql_common = "from `{$g5['g5_shop_item_use_table']}` a join `{$g5['g5_shop_item_table']}` b on (a.it_id=b.it_id)
						where a.is_confirm = '1' and b.it_use = '1' and b.it_soldout <> '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term ";		
	} else if($mode == 'qa') {
...
function apms_banner_event_rows($arr) {
	...
	$no_img = (isset($arr['no_img']) && $arr['no_img']) ? true : false;
	...
	for ($i=0; $row=sql_fetch_array($result); $i++) { 
		...
		if(!$no_img && !is_file(G5_DATA_PATH.'/event/'.$row['ev_id'].'_'.$banner)) continue;
		...
		$list[$j]['img'] = $img['src'];
		$list[$j]['alt'] = get_text($list[$j]['subject']);
		$j++;
	}
...
function apms_banner_rows($arr) {
	...
	for ($i=0; $row=sql_fetch_array($result); $i++) { 
		...
		if(!$no_img && !is_file(G5_DATA_PATH.'/banner/'.$row['bn_id'])) continue;

		$list[$j] = $row;
		$img = apms_thumbnail(G5_DATA_URL.'/banner/'.$row['bn_id'], $thumb_w, $thumb_h, false);
		$list[$j]['img'] = $img['src'];
		$list[$j]['alt'] = get_text($row['bn_alt']);
