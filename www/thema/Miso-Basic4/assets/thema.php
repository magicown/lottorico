...
// 탭이동
function tab_href($url) {
	if(!$url || G5_IS_MOBILE) return;
	$str = " onclick=\"go_page('".$url."');\"";
	return $str;
}
...
$is_top_menu = $at_set['tmenu'];
$is_content_menu = $at_set['cmenu'];
...
// 상단메뉴
$menu_top_list = array();
$menu_top_tmp = explode(",", $is_top_menu);
for($i=0; $i < count($menu_top_tmp);$i++) {
	$menu_top_item = trim($menu_top_tmp[$i]);
	if($menu_top_item) {
		$menu_top_list[] = $menu_top_item;
	}
}
$is_top_menu = (count($menu_top_list)) ? true : false;

// 컨텐츠메뉴
$menu_content_list = array();
$menu_content_tmp = explode(",", $is_content_menu);
for($i=0; $i < count($menu_content_tmp);$i++) {
	$menu_content_item = trim($menu_content_tmp[$i]);
	if($menu_content_item) {
		$menu_content_list[] = $menu_content_item;
	}
}
$is_content_menu = (count($menu_content_list)) ? true : false;

// 메뉴높이
$is_menuh = (isset($at_set['mh']) && $at_set['mh'] > 0) ? $at_set['mh'] : 110;
...
// 컨텐츠메뉴 너비
$is_cmenuw = (isset($at_set['cmw']) && $at_set['cmw'] > 0) ? $at_set['cmw'] : 170;

// 컨텐츠메뉴 높이
$is_cmenuh = (isset($at_set['cmh']) && $at_set['cmh'] > 0) ? $at_set['cmh'] : 400;

// 컨텐츠메뉴 영역
$is_cmenus = (isset($at_set['cms']) && $at_set['cms'] > 0) ? $at_set['cms'] : 860;
...
<style>
	...
	.pc-menu .nav-top.nav-float .menu-a, .pc-menu .nav-mega .menu-a { padding:0px <?php echo $is_menup;?>px; }
	.pc-menu .nav-top.nav-float .sub-1div::before { left: <?php echo $is_menup;?>px; }
	.pc-menu .nav-full-first, .pc-menu .nav-full-last, .pc-menu .nav-full.on .sub-1div { height: <?php echo $is_menuph;?>px; }
	.pc-menu .nav-full.on, .pc-menu .nav-full.on .at-container { height:<?php echo ($is_menuh + $is_menuph);?>px; }
	.pc-menu .subm-w { width:<?php echo $is_subw;?>px; }
	.pc-menu .nav-content, .pc-menu .nav-cw { width:<?php echo $is_cmenuw;?>px; }
	.pc-menu .nav-content-height { height:<?php echo $is_cmenuh;?>px; }
	.pc-menu .nav-content .sub-content { width:<?php echo $is_cmenus;?>px; left:<?php echo $is_cmenuw;?>px; }
