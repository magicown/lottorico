...
// 탈퇴회원수
$sql = " select count(*) as cnt from g5_member where 1 and mb_leave_date <> '' {$sql_order} ";
$row = sql_fetch($sql);
$leave_count = $row['cnt'];
...
$sql = "SELECT COUNT(*) AS cnt FROM g5_member WHERE mb_10 IN ('1','2') AND mb_leave_date = ''";
$free_cnt = sql_fetch($sql);

$sql = "SELECT COUNT(*) AS cnt FROM g5_member WHERE mb_10 > '2' AND mb_leave_date = ''";
$pay_cnt = sql_fetch($sql);
...
<div class="local_ov01 local_ov">
    ...
    <span class="btn_ov01"><span class="ov_txt">유료회원수 </span><span class="ov_num"> <?php echo number_format($pay_cnt['cnt']) ?>명 </span></span>
    <span class="btn_ov01"><span class="ov_txt">무료회원수 </span><span class="ov_num"> <?php echo number_format($free_cnt['cnt']) ?>명 </span></span>
...
<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    ...
    등급 :
    <select name="member_grade1" id="member_grade1">
        ...
        while($arr = sql_fetch_array($res)){
            ?>
            <option value="<?php echo $arr['idx']; ?>" <?php echo ($arr['idx']==$member_grade1)?'selected':''; ?>><?php echo $arr['pay_name']; ?>(<?php echo number_format($arr['pay_money']); ?>)</option>
        <?php } ?>
    </select>
    ...
    if($_SESSION['ss_mb_level']>=9){
        ...
        <div style="margin-left:30px; display: inline;">
            <a href="./member_list_excel.php?stx=<?php echo $stx; ?>&sfl=<?php echo $sfl; ?>&fr_date=<?php echo $fr_date; ?>&to_date=<?php echo $to_date; ?>&member_grade1=<?php echo $member_grade1; ?>" class="btn btn_03">엑셀다운로드</a>
            (회원검색 후 엑셀다운로드 하세요.)
        </div>
    <?php } ?>
</form>
...
<form name="fmemberlist" id="fmemberlist" action="./member_list_update.php" onsubmit="return fmemberlist_submit(this);" method="post">
    ...
    <input type="hidden" name="member_grade1" value="<?php echo $member_grade1 ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        <table>
            ...
            <tr>
                ...
                <th scope="col" id="mb_list_grp">등급변경일자</th>
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td headers="mb_list_cert"  rowspan="2" class="td_mbcert">
                        <a href="./member_send_now_form.php?mb_id=<?php echo $row['mb_id']; ?>" data-mb_name="<?php echo $row['mb_name']; ?>" data-mb_id="<?php echo $row['mb_id']; ?>" class="btn btn_03 member_send_now" data-backdrop="static" data-toggle="modal" data-target="#send_sms_now"  >즉시전송</a><span style="color:#000;">
                            <?php if($row['mb_10']>'1'){ ?>
                            <br>
                            (<?php echo $remind_lotto_cnt;?>조합)
                            <?php } ?>
                            <br>
                            <?php echo $str; ?></span>
                    </td>
                    ...
                    <td headers="mb_list_auth" class="td_mbstat">
                        ...
                        if($row['mb_5']=='Y')   echo '<span style="color:red; display: block; font-weight: bold;">(일시정지)</span>';
                        ?>
                    </td>
                    <td headers="mb_list_mobile" class="td_tel"><?php echo get_text($row['mb_hp']); ?><a data-info="<?php echo $row['mb_name']." ".$row['mb_hp']; ?>" class="btn btn_02 copy-info">이름/전화 복사</a></td>
                    <td headers="mb_list_lastcall" class="td_date"><?php echo substr($row['mb_today_login'],2,8); ?></td>
                    <td headers="mb_list_grp" class="td_numsmall"><?php echo $row['member_grade_date'] ?></td>
                    <td headers="mb_list_mng" rowspan="2" class="td_mng td_mng_s"><?php echo $s_mod ?><?php echo $s_grp ?></td>
                </tr>
                <tr class="<?php echo $bg; ?>">
                    <td headers="mb_list_name" class="td_mbname"><?php echo get_text($row['mb_name']); ?>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, '?'.$qstr.'&amp;page=&fr_date='.$fr_date.'&to_date='.$to_date.'&member_grade1='.$member_grade1); ?>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.member_bank_sms').on('click',function() {
            ...
            if (hp == '') {
                swal('', '등록된 휴대폰 번호가 없습니다.', 'warning');
                return false;
            }

            if (id == '') {
                swal('', '등록된 아이디가 없습니다.', 'warning');
                //alert('등록된 아이디가 없습니다.');
                return false;
            }

            if (bank == '') {
                swal('', '등록된 은행계좌가 없습니다.', 'warning');
                //alert('등록된 은행계좌가 없습니다.');
                return false;
            }

            swal({
                text: name + "님에게 계좌문자를 발송 하시겠습니까?",
                type: "info",
