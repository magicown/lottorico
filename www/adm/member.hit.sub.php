...
<div class="sch_last">
    ...
    <select name="lotto_turn" id="">
        ...
        <?php
            $que = "SELECT hit_turn FROM lotto_hit_result WHERE 1 ORDER BY hit_turn DESC";
            $res = sql_query($que);
            while($arr = sql_fetch_array($res)){
        ?>
                <option value="<?php echo $arr['hit_turn'];?>" <?php echo ($arr['hit_turn']==$_GET['lotto_turn'])?'selected':''; ?>><?php echo $arr['hit_turn'];?></option>
        <?php
            }
        ?>
    </select>
