...
function admin_referer_check($return=false)
{
    ...
    if($p['path']) {
		if(preg_match( '/\/'.preg_quote(G5_ADMIN_DIR).'\//i', $p['path']) || preg_match( '/\/'.preg_quote(G5_BBS_DIR).'\//i', $p['path'])){
			;
		} else {
	        $msg = '올바른 방법으로 이용해 주십시오';
		}
...
// 회원아이디를 SELECT 형식으로 얻음
function get_pay_type_select($selected="")
{
    global $g5;

    $sql = " select * from g5_pay_type where 1 ";
    $result = sql_query($sql);
    $str = '<select name="pay_type" id="pay_type"><option value="n">::결제선택::</option>';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['idx'].'"';
        if ($row['idx'] == $selected) $str .= ' selected';
        $str .= '>'.$row['pay_name'].'('.number_format($row[pay_money]).'원) - '.$row['pay_memo'].'</option>';
    }
    $str .= '</select>';
    return $str;
}
...
// 회원아이디를 SELECT 형식으로 얻음
function get_pay_type_select_change($no, $selected="")
{
    global $g5;

    $sql = " select * from g5_pay_type where 1 ";
    $result = sql_query($sql);
    $str = '<select name="pay_type" id="pay_type'.$no.'"><option value="n">::변경등급선택::</option>';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['idx'].'"';
        if ($row['idx'] == $selected) $str .= ' selected';
...
// 상품기간설정
function get_pay_date_select($selected="")
{
    global $g5;
    $month = array(1,3,6,12,24,36,48,50,60);
    $str = '<select name="pay_date" id="pay_date"><option value="n">::기간선택::</option>';
    for ($i=0; $i<count($month); $i++)
    {
        $str .= '<option value="'.$month[$i].'"';
        if ($month[$i] == $selected) $str .= ' selected';
        $str .= '>'.$month[$i].'개월</option>';
    }
    $str .= '</select>';
    return $str;
}
...
//상품 만료일 만들기
function makeGradeExpireDate($idx){
    $que = "SELECT pay_date FROM g5_pay_type WHERE idx = {$idx}";
    //echo $que;
    $row = sql_fetch($que);
    return $today = date("Y-m-d",strtotime("-1 day",strtotime("+{$row['pay_date']} months")));
}


//등급명 가져오기
function getGrade($idx){
    $que = "SELECT * FROM g5_pay_type WHERE idx = {$idx}";
    //echo $que;
    $rs = sql_fetch($que);
    return $rs;
}
...
//등록된 제외수를 가져온다.
function getExceptNum($turn, $idx=''){
    $que = "SELECT * FROM g5_except_num WHERE turn = {$turn} AND pay_idx = '{$idx}'";
    //echo $que;
    $row = sql_fetch($que);
    return $row['except_num'];
}
//회원 정보 가져오기
function get_member_info($mb_id){
