...
if (isset($_REQUEST['fr_date']))  { // search text (검색어)
    $fr_date = get_search_string(trim($_REQUEST['fr_date']));
    if ($fr_date)
...
if (isset($_REQUEST['to_date']))  { // search text (검색어)
    $to_date = get_search_string(trim($_REQUEST['to_date']));
    if ($to_date)
        $qstr .= '&amp;fr_date=' . urlencode($to_date);
} else {
    $to_date = '';
}

if (isset($_REQUEST['member_grade1']))  { // search text (검색어)
    $member_grade1 = get_search_string(trim($_REQUEST['member_grade1']));
    if ($member_grade1)
        $qstr .= '&amp;member_grade1=' . urlencode($stx);
} else {
    $member_grade1 = '';
}
if (isset($_REQUEST['stx']))  { // search text (검색어)
